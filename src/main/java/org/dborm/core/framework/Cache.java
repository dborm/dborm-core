...
import org.dborm.core.annotation.AnnotationUtils;
...
public class Cache {
    ...
    public TableBean getTablesCache(Class<?> entityClass) {
        ...
        if (table == null) {//如果缓存中不存在则从该类的注解中解析信息，如果解析出信息则添加到缓存中，否则抛出异常
            ...
            if (table != null) {
                ...
                throw new RuntimeException("无法获得类:" + entityClass.getName() + " 对应的表信息，请检查该类是否添加Table注解,或者使用xml描述该类对应的表信息！");
            }
        }
        return table;
    }
    ...
    /**
     * 缓存面向对象的SQL语句，如对整个对象的新增、替换及删除的SQL语句，所需参数用?代替<br>
     * 键：类名+操作符（如com.dborm.Login.DELETE：对Login对象的删除语句、com.dborm.Login.REPLACE：
     * 对Login对象的替换语句）<br>
     * 值：SQL语句<br>
     * 动态缓存
     */
